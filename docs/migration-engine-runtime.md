...
v2.2.0부터 **`engine_mode` 필드는 deprecated**. capability 폴백 체인이 대체한다.

| v2.1.x 이하 | v2.2.0+ |
|------------|---------|
| `engine_mode: codex_analysis` | `capabilities.codex_cli: true` |
| `engine_mode: opus_analysis` | `capabilities.opus_review: true` (codex 없을 때 폴백) |
| 분석=Codex 또는 Opus | 분석은 항상 Claude(`orchestrator_model`) |
| 구현=Sonnet 또는 Codex | 구현은 항상 Claude(`orchestrator_model`) |
| 교차 검증=Codex 또는 Opus | REVIEW 폴백 체인: codex → opus_advisor → self_review |

**마이그레이션 방법**: `/sprote:setup` 재실행 또는 수동으로 `engine.json` 갱신:
...
`engine_mode` 필드가 남아있어도 동작은 정상이지만 한 줄 안내가 출력된다.

---

## 이하 v1.16 마이그레이션 (legacy)

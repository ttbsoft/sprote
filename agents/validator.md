...
- [ ] `capabilities`(codex_cli·opus_review·subagents) 설명 포함
- [ ] REVIEW 폴백 체인 3순위 설명 포함 (codex → opus_advisor → self_review)
- [ ] `engine_mode` 필드 deprecated 안내 포함 (있다면)
...
### 오케스트레이터 검증
- [ ] **Phase -0.5 (엔진 해석) 블록 포함 여부** — 누락 시 FAIL (TTB 표준 §9.4)
- [ ] Phase -0.5 블록이 `engine.json` 읽기 + ANALYSIS·IMPLEMENTATION·REVIEW 변수 할당 + 폴백 체인(§9.3) 적용을 모두 포함
- [ ] Phase 0 (컨텍스트 확인) 포함 여부
- [ ] 에이전트 구성표 존재 + 각 에이전트에 `role` (ANALYSIS/IMPLEMENTATION/REVIEW/LEADER/LIGHT) 표시
- [ ] 에러 핸들링 섹션 존재 (REVIEW 폴백 강등 항목 포함 — §9.6, §9.7)
- [ ] 테스트 시나리오 3종 포함 (codex 정상 / opus_advisor 강등 / self_review 단독)

### 모델 설정 검증 (TTB 표준 §6, §9)
- [ ] ANALYSIS·IMPLEMENTATION Agent 호출이 `{ANALYSIS_*}` / `{IMPLEMENTATION_*}` 변수 사용
- [ ] REVIEW Agent 호출이 `{REVIEW_MODE}` 분기 처리 + `{REVIEW_*}` 변수 사용
- [ ] LEADER 역할 Agent 호출이 `general-purpose` + `sonnet` 고정
- [ ] LIGHT 역할 Agent 호출이 `general-purpose` + `haiku` 고정
- [ ] 분석·구현·리뷰 호출에 모델 하드코딩(`model: "opus"` 등 리터럴) 없음 → 발견 시 FAIL
